...
#
#  Run once:  chmod +x tmux-setup.sh && ./tmux-setup.sh
#  Then:      source ~/.zshrc
#  Edit:      tm edit    (opens your session config)
#  Go:        tm up      (starts everything)
#
#  Zero external dependencies — uses macOS built-in python3.
# ═══════════════════════════════════════════════════════════════

set -e

PODMAN_PATH=$(which podman 2>/dev/null || echo "/opt/homebrew/bin/podman")
...
set -g status-right '#[fg=#00d4ff] #(${PODMAN_PATH} ps -q 2>/dev/null | wc -l | tr -d " ") containers  %H:%M '
  ...
  "sessions": [
    {
      ...
      "windows": [
        ...
        },
        {
          ...
          "path": "~/projects/infra",
          "commands": [
            "podman-compose -f docker-compose.infra.yml restart my-gateway",
            "podman logs -f my-gateway"
          ]
        }
      ]
    },
      ...
      "windows": [
        {
          ...
          "commands": [
            ...
            "podman-compose -f docker-compose.mfe.yml up -d"
          ]
        },
        {
          "name": "host",
      ...
      "windows": [
        {
          "name": "containers",
          "commands": [
            "watch -n 5 \"podman ps --format 'table {{.Names}}\\t{{.Status}}\\t{{.Ports}}' | sort\""
          ]
        },
        {
          "name": "resources",
...
tm() {
  case "${1:-help}" in
    ...
    up)
      ...
      echo "  tm <name>        attach to a session"
...
logs() {
  local container=$(podman ps --format '{{.Names}}' 2>/dev/null | fzf --prompt="logs> " --height=40%)
  [ -z "$container" ] && return
  if [ -n "$TMUX" ]; then
    tmux split-window -h "podman logs -f $container"
  else
...
recap() {
  ...
  echo ""
  echo "─── Containers ──"
  podman ps --format "  {{.Names}}: {{.Status}}" 2>/dev/null | sort || echo "  none"
  echo ""
  echo "─── Ports ───────"
  podman ps --format "  {{.Names}}: {{.Ports}}" 2>/dev/null | grep -v ": $" | sort -u || echo "  none"
}
